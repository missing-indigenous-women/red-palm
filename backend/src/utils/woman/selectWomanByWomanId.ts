...
export async function selectWomanByWomanId(womanId: string) : Promise<Woman|null> {
    try {
        ...
        const mySqlQuery = 'SELECT BIN_TO_UUID(womanId) AS womanId,womanAliases ,womanDateOfDisappearence,womanDateOfBirth, womanEyeColor, womanFavoriteHangoutPlaces,womanFirstName,womanHairColor, womanHeight,womanHobbiesAndInterests,womanIdentifyingMarks,WomanLastName, womanLastLocation,womanLatitude,womanLongitude, womanPhoto1, womanTribe,womanWeight FROM woman WHERE womanId = UUID_TO_BIN (:womanId)'
