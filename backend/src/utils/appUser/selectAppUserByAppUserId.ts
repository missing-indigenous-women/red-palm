...
export async function selectAppUserByAppUserId(appUserId: string) : Promise<AppUser|null> {
    try {
        ...
        const mySqlQuery = 'SELECT BIN_TO_UUID(appUserId) AS appUserId,appUserActivationToken, appUserDisplayName,appUserEmail,appUserFirstName, appUserHash, appUserLastName, appUserAvatarUrl FROM appUser WHERE appUserId = UUID_TO_BIN (:appUserId)'
